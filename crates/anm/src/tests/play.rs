...
use crate::play::{AnmPlayer, EPSILON_S};
...
fn determinism_same_ticks_same_scenes() {
    let mut a = AnmPlayer::new(demo()).unwrap();
    let mut b = AnmPlayer::new(demo()).unwrap();
    a.play();
    b.play();
...
fn scrub_equals_play_until_t() {
    let mut played = AnmPlayer::new(demo()).unwrap();
    played.play();
    for _ in 0..23 {
        played.tick(&tick(0.029));
        let mut scrubbed = AnmPlayer::new(demo()).unwrap();
...
fn validity_window_no_research_between_boundaries() {
    let mut p = AnmPlayer::new(two_windows()).unwrap();
    p.play();
    p.tick(&tick(0.01));
    p.scene();
...
fn signals_reflect_play_pause_scrub() {
    let mut p = AnmPlayer::new(demo()).unwrap();
...
fn hold_easing_keeps_from_value_until_segment_end() {
    ...
    let mut p = AnmPlayer::new(t).unwrap();
...
fn decoded_file_plays_like_the_authored_timeline() {
    ...
    let mut authored = AnmPlayer::new(demo()).unwrap();
