//! bench vs lottie (spec gate, doc/anm-format-v0.md): IR timelines that
...
//! its gzip -9 (the spec ruler: anm born at or below the gzip size).
//!
...
//!   each (st), driven by 6 expressions and loopOut('cycle'). anm v0
...
//!   time remap), scale pop (2 kfs), 2 extra shapes. anm v0 has no
...
fn report(b: &Bench) {
    let bps = b.anm_bytes as f32 / b.duration_s;
    ...
    println!("  anm    {:>6} B  {:>8.1} B/s", b.anm_bytes, bps);
    println!("  json   {:>6} B  {:>8.1} B/s", b.json_bytes, json_bps);
    println!("  gzip-9 {:>6} B  {:>8.1} B/s", b.gzip_bytes, gz_bps);
    println!(
        "  anm/json {:.3}x   anm/gzip {:.3}x   gate(anm<=gzip): {}",
        b.anm_bytes as f32 / b.json_bytes as f32,
        b.anm_bytes as f32 / b.gzip_bytes as f32,
        if b.anm_bytes <= b.gzip_bytes {
            "PASS"
        } else {
            "FAIL"
        }
