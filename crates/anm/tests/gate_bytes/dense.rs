...
//! from ref/lottie at test time, map honestly to anm v0 IR and encode
...
//! paths, 12045 vertices, 334 paints). anm mapping: one snapshot
...
pub fn report(b: &Dense) {
    ...
    println!("== {} ==", b.name);
    println!("  anm raw {:>7} B  {:>9.1} B/s", b.raw, bps(b.raw));
    let delta = b.opt as i64 - b.raw as i64;
    println!(
        "  anm opt {:>7} B  {:>9.1} B/s  (optimizer {delta:+} B)",
        b.opt,
        bps(b.opt)
    );
    ...
    println!(
        "  anm/json {:.3}x  anm/gzip {:.3}x  anm/webm {:.3}x  gate(anm<=gzip): {}",
...
pub fn encode_pair(tl: &Timeline, assets: &[Asset]) -> (usize, usize, Timeline) {
    ...
    decode(&opt).expect("optimized anm decodes back");
