...
use anm::{
    AnmPlayer, DiscoverConfig, Easing, Node, NodeKind, OptimizeCfg, Prop, Props, Value, decode,
...
fn mode_b_discover_optimize_encode_decode_play_round_trip() {
    ...
    let mut player = AnmPlayer::new(doc.timeline).expect("decoded timeline plays");
    ...
    let segs = |tl: &anm::Timeline| tl.tracks.iter().map(|t| t.segments.len()).sum::<usize>();
