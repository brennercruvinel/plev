//! Recently-opened .urna files, persisted as a plain JSON array.
//!
//! Location follows the urna CLI's data-home convention: `$XDG_DATA_HOME/
//! urnaui/recents.json`, else `~/.local/share/urnaui/recents.json`. No
...
pub fn default_path() -> Option<PathBuf> {
    ...
    Some(data_home.join("urnaui").join("recents.json"))
}
...
mod tests {
    ...
    fn missing_file_loads_empty() {
        let path = std::env::temp_dir().join(format!("urnaui_recents_none_{}", std::process::id()));
        assert!(load(&path).is_empty());
    }

    #[test]
    fn record_dedupes_caps_and_roundtrips() {
        let dir = std::env::temp_dir().join(format!("urnaui_recents_{}", std::process::id()));
        let path = dir.join("recents.json");

        for i in 0..12 {
            record(&path, &format!("/db/{i}.urna"));
        }
        let recents = load(&path);
        assert_eq!(recents.len(), MAX_RECENTS);
        // Most recent first, oldest evicted.
        assert_eq!(recents[0], "/db/11.urna");
        assert_eq!(recents[MAX_RECENTS - 1], "/db/2.urna");

        // Re-opening an existing entry moves it to the front.
        record(&path, "/db/5.urna");
        let recents = load(&path);
        assert_eq!(recents[0], "/db/5.urna");
        assert_eq!(
            recents.iter().filter(|p| *p == "/db/5.urna").count(),
            1,
            "no duplicates"
        );
