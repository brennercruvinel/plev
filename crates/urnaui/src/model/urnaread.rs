//! Portable, dependency-free `.urna` reader over in-memory bytes — the
//! web backend's foundation, compiled and unit-tested on every target.
//!
//! Why a reimplementation: `urna-runtime` needs mmap + rayon and
//! `urna-format` pulls in `zstd` (a C build), none of which compile to
//! wasm32-unknown-unknown, and this phase must not patch the urna repo.
...
// Section ids (mirror urna_format::layout; the view model never names
// urna crates).
...
/// intpack repack kinds (urna_format::sections).
...
/// An in-memory parsed `.urna` file.
pub struct UrnaBytes {
    name: String,
    bytes: Vec<u8>,
    /// The header magic as found (`URNA`, or `NEST` on pre-rename files).
    magic: String,
    version_major: u16,
...
impl UrnaBytes {
    ...
    pub fn open(name: String, bytes: Vec<u8>) -> Result<Self> {
        if bytes.len() < HEADER_SIZE + FOOTER_SIZE {
            return Err("file too small for a .urna header + footer".to_string());
        }
        let mut h = Cursor::new(&bytes[..HEADER_SIZE]);
        // `URNA` is the current magic; `NEST` is what files written before
        // the rename carry, and the urna reader still accepts it.
        let magic = h.take(4)?;
        if magic != b"URNA" && magic != b"NEST" {
            return Err("bad magic: not a .urna file".to_string());
        }
        ...
        let hits = scores[..k_usize.min(n)]
            ...
            .map(|&(i, score)| {
                ...
                SearchHitView {
                    ...
                    citation_id: format!("urna://{}/{}", self.content_hash, self.chunk_ids[i]),
                    reranked: false,
                }
            })
            .collect::<Vec<_>>();
