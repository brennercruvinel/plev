//! urnaui library target: the .urna explorer as a reusable crate.
//!
...
//! iOS (staticlib `urnaui_ios_main`) all drive the same app.
//!
//! The model compiles on every target: pure modules (`types`, `urnaread`,
//! `graph`, `bench`) everywhere; the mmap backend and worker thread are
//! native-only, the web build swaps in `urnaread` + an inline worker.

// Let the view modules keep referring to this crate by name
// (`urnaui::model::...`) now that they live in the library.
extern crate self as urnaui;
