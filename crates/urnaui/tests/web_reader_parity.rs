//! Parity tests for the portable web reader (`model::urnaread`): files
//! are built with the real `urna_format` writer and cross-checked against
//! `urna_runtime` (the mmap ground truth), so the GUI-side port is proven
...
fn both(bytes: &[u8]) -> (UrnaBytes, UrnaBackend) {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("parity.urna");
    std::fs::write(&path, bytes).unwrap();
    let native = UrnaBackend::open(&path).unwrap();
    std::mem::forget(dir);
    (
        UrnaBytes::open("parity.urna".into(), bytes.to_vec()).unwrap(),
        native,
    )
}
...
fn identity_chunks_and_inspect_match_the_runtime() {
    let bytes = UrnaFileBuilder::new(demo_manifest())
...
fn float16_embeddings_decode_at_parity() {
    let bytes = UrnaFileBuilder::new(demo_manifest())
        .add_chunks(demo_chunks())
        .embedding_dtype(urna_format::EmbeddingDType::Float16)
        .build_bytes()
        .unwrap();
    let (web, native) = both(&bytes);
...
fn hostile_and_unsupported_inputs_error_cleanly() {
    // Garbage.
    assert!(UrnaBytes::open("x".into(), vec![0u8; 16]).is_err());
    // A zstd-compressed text section is a clean "desktop only" error, not
    // a panic.
    let bytes = UrnaFileBuilder::new(demo_manifest())
        .add_chunks(demo_chunks())
        .text_encoding(urna_format::SectionEncoding::Zstd)
        .build_bytes()
        .unwrap();
    let result = UrnaBytes::open("z.urna".into(), bytes);
...
fn benchmark_runs_on_the_web_reader() {
    let bytes = UrnaFileBuilder::new(demo_manifest())
        ...
        .build_bytes()
        .unwrap();
    let web = UrnaBytes::open("b.urna".into(), bytes).unwrap();
