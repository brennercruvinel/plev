//! Integration tests for `UrnaBackend` against real .urna files: a
//! synthetic corpus built with `urna_format::UrnaFileBuilder` (always
...
fn synthetic_corpus() -> PathBuf {
    ...
    let path = dir.path().join("synthetic.urna");
...
fn opens_searches_and_decodes_a_synthetic_corpus() {
    let backend = UrnaBackend::open(synthetic_corpus()).unwrap();
...
/// GUI-side (via UrnaView + CsrIndex, mirroring the runtime's open gate).
#[test]
fn loads_the_csr_graph_when_the_capability_is_set() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("graphed.urna");
    let manifest = Manifest {
        embedding_model: "urnaui-test".into(),
        embedding_dim: 4,
        n_chunks: 3,
    ...
    let edges = [
        Edge {
            src: 0,
            dst: 1,
            edge_type: urna_format::EDGE_TYPE_NEXT_CHUNK,
        },
        Edge {
            src: 1,
            dst: 2,
        ...
        Edge {
            src: 0,
            dst: 2,
            edge_type: urna_format::EDGE_TYPE_SEMANTIC,
        },
    ];
    ...
    let backend = UrnaBackend::open(&path).unwrap();
    let view = backend.opened_view().unwrap();
    assert!(view.has_graph);
    ...
    assert_eq!(graph.edge_type(0, 1), Some(urna_format::EDGE_TYPE_SEMANTIC));
...
/// The shared corpus fixture lives in the sibling urna workspace. It is a
...
fn opens_the_shared_fixture_when_present() {
    let path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../../urna/dat/corpus_next.v1.urna");
    let Ok(backend) = UrnaBackend::open(&path) else {
        eprintln!("skipping: {path:?} is absent or not a valid .urna (git-lfs pointer?)");
        return;
    };
...
/// Dev tool: `cargo test -p urnaui -- --ignored make_demo_fixture` writes a
/// small demo corpus to `target/tmp/urnaui_demo.urna` so the real app can
/// be run against it (`cargo run -p urnaui -- target/tmp/urnaui_demo.urna`).
/// Ignored by default; the output path is gitignored build territory.
#[test]
#[ignore = "dev tool: writes target/tmp/urnaui_demo.urna"]
fn make_demo_fixture() {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../target/tmp/urnaui_demo.urna");
    std::fs::create_dir_all(path.parent().unwrap()).unwrap();
    let manifest = Manifest {
        embedding_model: "urnaui-demo".into(),
        embedding_dim: 8,
        n_chunks: 6,
        chunker_version: "urnaui-demo/1".into(),
        model_hash: format!("sha256:{}", "ab".repeat(32)),
        title: Some("urnaui demo corpus".into()),
        ..Default::default()
    };
    let texts = [
    ...
    let big_path = path.with_file_name("urnaui_demo_big.urna");
    let n = 300usize;
    let manifest = Manifest {
        embedding_model: "urnaui-demo".into(),
        embedding_dim: 8,
        ...
        chunker_version: "urnaui-demo/1".into(),
        model_hash: format!("sha256:{}", "ab".repeat(32)),
        title: Some("urnaui demo corpus (big)".into()),
        ..Default::default()
    };
