...
fn place_replace_remove_act_at_the_authored_instants() {
    let mut p = MonsterPlayer::new(ops_timeline()).unwrap();
...
fn track_on_a_placed_node_animates_from_the_placed_value() {
    ...
    let mut p = MonsterPlayer::new(tl).unwrap();
...
fn playback_with_ops_is_deterministic_and_scrub_consistent() {
    let mut played = MonsterPlayer::new(ops_timeline()).unwrap();
    played.play();
    for _ in 0..23 {
        ...
        let mut scrubbed = MonsterPlayer::new(ops_timeline()).unwrap();
...
fn decoded_ops_file_plays_like_the_authored_timeline() {
    ...
    let mut authored = MonsterPlayer::new(ops_timeline()).unwrap();
