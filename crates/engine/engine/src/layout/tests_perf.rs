...
fn test_1000_nodes_under_1ms() {
    ...
    // perf smoke guard. a single wall-clock sample is flaky under concurrent
    // load (a busy machine can inflate one run past the bound), so take the
    // best of several: load only ever adds time, so the minimum reflects the
    // true compute cost. compute() clears and rebuilds the tree on every call,
    // so each run does the full work and no caching skews the minimum. real
    // perf tracking lives in the benches (cargo bench).
    ...
    assert!(
        best < threshold,
        "Layout of 1001 nodes took {best:?} (best of 5, > {threshold:?})"
    );
}
