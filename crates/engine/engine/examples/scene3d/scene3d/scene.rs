...
fn clinic_rooms() -> Vec<Room> {
    vec![
        Room {
            ...
            h: 2.8,
            ty: "clinical",
            furniture: &["maca", "mesa", "cadeira", "armario"],
        },
        Room {
            x: 4.0,
            z: 5.0,
...
impl ClinicScene {
    pub fn build(device: &wgpu::Device, queue: &wgpu::Queue) -> Self {
        ...
        opaque.verts.push(Vertex3D {
            position: [5.0, 0.1, 5.0],
            normal: n_up,
            color: bright,
        });
        opaque.verts.push(Vertex3D {
            position: [15.0, 0.1, 5.0],
            normal: n_up,
            color: bright,
        });
        opaque.verts.push(Vertex3D {
            position: [10.0, 0.1, 15.0],
            normal: n_up,
            color: bright,
        });
        ...
        let opaque_vb = make_buf(
            device,
            queue,
            bytemuck::cast_slice(&opaque.verts),
            wgpu::BufferUsages::VERTEX,
        );
        let opaque_ib = make_buf(
            device,
            queue,
            bytemuck::cast_slice(&opaque.idxs),
            wgpu::BufferUsages::INDEX,
        );
        let transparent_vb = make_buf(
            device,
            queue,
            bytemuck::cast_slice(&trans.verts),
            wgpu::BufferUsages::VERTEX,
        );
        let transparent_ib = make_buf(
            device,
            queue,
            bytemuck::cast_slice(&trans.idxs),
            wgpu::BufferUsages::INDEX,
        );
        let line_vb = make_buf(
            device,
            queue,
            bytemuck::cast_slice(&lines.verts),
            wgpu::BufferUsages::VERTEX,
        );

        eprintln!(
            "[scene3D] opaque: {} verts, {} idx | trans: {} verts, {} idx | lines: {} verts | center: {:?}",
            opaque.verts.len(),
            opaque.idxs.len(),
            trans.verts.len(),
            trans.idxs.len(),
        ...
        Self {
            opaque_vb,
            opaque_ib,
            opaque_count: opaque.idxs.len() as u32,
            transparent_vb,
            transparent_ib,
            transparent_count: trans.idxs.len() as u32,
            line_vb,
            line_count: lines.verts.len() as u32,
            center,
        }
    }
}
