...
fn sans(font_size: f32) -> TextStyle {
...
fn proportional_font_iiii_narrower_than_wwww() {
    let (narrow, _) = TextMeasurer::measure_styled("iiii", &sans(16.0), None);
    let (wide, _) = TextMeasurer::measure_styled("WWWW", &sans(16.0), None);
    assert!(narrow > 0.0);
    assert!(
        narrow < wide,
...
fn width_scales_with_font_size() {
    let (small, _) = TextMeasurer::measure_styled("Hello", &sans(12.0), None);
    let (large, _) = TextMeasurer::measure_styled("Hello", &sans(24.0), None);
...
// When only the 400 face was embedded, any text at weight 500/600/700 skipped
// the UI family entirely and resolved per-word through the platform fallback
// lists — on macOS: Apple SD Gothic Neo + Apple Color Emoji at 500/600
// (+35% advance), Menlo at 700 ("1,632" headlines, 600-weight labels).
...
fn assert_advance_close_to_regular(text: &str, style: &TextStyle, weight: u16) {
    ...
    // Sibling faces of one family differ by up to ~10% (Inclusive Sans Bold
    // runs ~5.3% wider than Inclusive Sans Regular); a broken family fallback
    // drifts ~35%+. The
...
fn semibold_digits_advance_close_to_regular() {
    ...
    assert_advance_close_to_regular("1,632", &sans(28.0), 600);
}

#[test]
fn medium_multiword_advance_close_to_regular() {
    assert_advance_close_to_regular("Expense Tracker", &sans(18.0), 500);
}
...
#[test]
fn inclusive_sans_family_resolves_its_faces_for_all_ui_weights() {
    for weight in [400u16, 500, 600, 700] {
        let style = sans(16.0).with_weight(weight);
        ...
        for (family, face_weight) in &faces {
            ...
            assert_eq!(
                *face_weight, weight,
                "weight {weight} resolved to Inclusive Sans face of weight {face_weight}"
            );
        }
    }
}
...
#[test]
fn default_family_resolves_inclusive_sans_faces_for_all_ui_weights() {
    // `font_family: None` maps to Family::SansSerif; the engine pins that to
    // the embedded Inclusive Sans so weights resolve deterministically on any
    // system.
    for weight in [400u16, 500, 600, 700] {
...
fn round_trip_proportional() {
    assert_round_trip("Hello, World!", &sans(16.0));
}
...
fn round_trip_narrow_chars_proportional() {
    // 'i' and 'l' are very narrow in Inclusive Sans — the old 0.6 ratio failed here.
    assert_round_trip("illiilli", &sans(16.0));
}
...
fn backend_measure_matches_measurer() {
    ...
    let runs = single_run("Hello", sans(16.0));
    let from_backend = backend.measure("Hello", &runs, None);
    let from_measurer = TextMeasurer::measure_styled("Hello", &sans(16.0), None);
...
fn letter_spacing_distinguishes_measure_cache() {
    ...
    let text = "Follow";
    let (w0, _) = TextMeasurer::measure_styled(text, &sans(14.0), None);
    let (w1, _) = TextMeasurer::measure_styled(text, &sans(14.0).with_letter_spacing(0.35), None);
    assert!(w1 > w0);
}
...
fn line_metrics_come_from_real_faces() {
    ...
    assert!(
        ...
        "Inclusive Sans ascent+descent (~1.20em) fits inside the 1.3 line box"
    );
...
fn truncate_keeps_text_that_fits() {
    let s = "short enough";
    assert_eq!(TextMeasurer::truncate_to_width(s, &sans(14.0), 500.0), s);
}

#[test]
fn truncate_empty_string_stays_empty() {
    assert_eq!(TextMeasurer::truncate_to_width("", &sans(14.0), 100.0), "");
}
