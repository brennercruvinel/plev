...
impl App {
    pub(crate) fn render(&mut self) {
        ...
        self.compositor.draw_text(
            TextNodeKey::from_style("DATA VISUALIZATION", &style(24.0, 30.0), Some(vw - 64.0)),
            24.0,
            12.0,
            TEXT,
        );
    ...
    fn build_line_chart(&mut self, cx: f32, cy: f32, half_w: f32, half_h: f32, r: f32) {
        ...
        self.compositor.draw_text(
            TextNodeKey::from_style("LINE CHART", &style(12.0, 16.0), None),
            cx + 12.0,
            cy + 8.0,
            ACCENT,
        );
        ...
        for i in 0..=4 {
            ...
            self.compositor.draw_text(
                TextNodeKey::from_style(&label, &style(9.0, 12.0), Some(32.0)),
                cx + 6.0,
                ly - 5.0,
                TEXT_DIM,
            );
    ...
    fn build_bar_chart(&mut self, cx: f32, cy: f32, half_w: f32, half_h: f32, r: f32) {
        ...
        self.compositor.draw_text(
            TextNodeKey::from_style("BAR CHART", &style(12.0, 16.0), None),
            cx + 12.0,
            cy + 8.0,
            GREEN,
        );
        ...
        for (i, label) in months.iter().enumerate().take(scaled.len()) {
            ...
            self.compositor.draw_text(
                TextNodeKey::from_style(label, &style(8.0, 11.0), Some(bar_w)),
                lx,
                chart_y + chart_h + 4.0,
                TEXT_DIM,
            );
    ...
    fn build_area_chart(&mut self, cx: f32, cy: f32, half_w: f32, half_h: f32, r: f32) {
        ...
        self.compositor.draw_text(
            TextNodeKey::from_style("STACKED AREA", &style(12.0, 16.0), None),
            cx + 12.0,
            cy + 8.0,
            PURPLE,
        );
        ...
        // Area 2 (back)
        {
            ...
            let mut b = engine::path::PathBuilder::new();
        ...
        self.compositor.draw_text(
            TextNodeKey::from_style("Series A", &style(9.0, 12.0), None),
            cx + 24.0,
            ly - 1.0,
            TEXT_DIM,
        );
        ...
        self.compositor.draw_text(
            TextNodeKey::from_style("Series B", &style(9.0, 12.0), None),
            cx + 92.0,
            ly - 1.0,
            TEXT_DIM,
        );
    ...
    fn build_pie_chart(&mut self, cx: f32, cy: f32, half_w: f32, half_h: f32, r: f32) {
        ...
        self.compositor.draw_text(
            TextNodeKey::from_style("PIE CHART", &style(12.0, 16.0), None),
            cx + 12.0,
            cy + 8.0,
            ORANGE,
        );
        ...
        self.compositor.draw_text(
            TextNodeKey::from_style("100%", &style(14.0, 18.0), Some(60.0)),
            pie_cx - 18.0,
            pie_cy - 8.0,
            TEXT,
        );
        ...
        for (i, (label, color)) in labels.iter().zip(pie_colors.iter()).enumerate() {
            ...
            self.compositor.draw_text(
                TextNodeKey::from_style(&pct, &style(10.0, 14.0), Some(half_w * 0.3)),
                lx + 16.0,
                ly,
                TEXT_DIM,
            );
...
fn style(size: f32, line_height: f32) -> engine::text::TextStyle {
    engine::text::TextStyle::new(size).with_line_height(line_height)
}
