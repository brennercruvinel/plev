//! GPU render pipeline for the builder demo.

use engine::view::{View, ViewContext};
...
pub(crate) fn render(app: &mut App) {
    ...
    let output = match surface.get_current_texture() {
        ...
        Err(engine::wgpu::SurfaceError::Timeout) => return,
        Err(e) => {
    ...
    let elem = ui::build_ui();
    for node in elem.render(&mut cx) {
    ...
    engine::window::resolve_layer_text(&mut app.compositor, gpu, text_system);
...

    {
        let mut pass = encoder.begin_render_pass(&engine::wgpu::RenderPassDescriptor {
            ...
            color_attachments: &[Some(engine::wgpu::RenderPassColorAttachment {
                ...
                ops: engine::wgpu::Operations {
                    load: engine::wgpu::LoadOp::Clear(engine::wgpu::Color {
                        // #0F0F1A sRGB, linearized for the sRGB surface encode-on-write
                        r: 0.0049,
                        g: 0.0049,
                        b: 0.0103,
                        a: 1.0,
