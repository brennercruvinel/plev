...
impl SnakeGame {
    ...
    fn build_header(&self, compositor: &mut Compositor, w: f32, _h: f32) {
        ...
        compositor.draw_text(
            TextNodeKey::new(title, 22.0, 28.0, None), 24.0, 10.0, TEXT_COLOR,
        );

        let score_text = format!("Score: {}   High: {}", self.score, self.high_score);
        compositor.draw_text(
            TextNodeKey::new(&score_text, 13.0, 18.0, None), 24.0, 34.0, TEXT_DIM,
        );
        ...
        compositor.draw_rounded_rect(RoundedRectParams {
            x: w - 24.0 - badge_w, y: 14.0, w: badge_w, h: 22.0,
            color: ACCENT, corner_radius: 4.0, border_width: 0.0, border_color: [0.0; 4],
        });
        compositor.draw_text(
            TextNodeKey::new(mode_label, 11.0, 14.0, Some(badge_w - 8.0)),
            w - 24.0 - badge_w + 4.0, 18.0, TEXT_COLOR,
        );
    }

    fn build_grid(
        &self, compositor: &mut Compositor,
        ox: f32, oy: f32, total_w: f32, total_h: f32, cell: f32,
    ) {
        ...
        if self.food_flash > 0.01 {
            compositor.draw_rect(ox, oy, total_w, total_h,
            ...
            for cx in 0..GRID_W {
                ...
                match cell_type {
                    ...
                    Cell::Snake => {
                        ...
                        compositor.draw_rounded_rect(RoundedRectParams {
                            x: px, y: py, w: sz, h: sz,
                            color, corner_radius: radius, border_width: 0.0, border_color: [0.0; 4],
                        });
                    }
                    Cell::Food => {
                        let glow_sz = sz + 6.0;
                        compositor.draw_rounded_rect(RoundedRectParams {
                            x: px - 3.0, y: py - 3.0, w: glow_sz, h: glow_sz,
                            color: FOOD_GLOW, corner_radius: radius * 2.0, border_width: 0.0, border_color: [0.0; 4],
                        });
                        let pulse = ((self.frame as f32 * 0.08).sin() * 0.5 + 0.5) * 0.15;
                        let food_c = [FOOD_COLOR[0], FOOD_COLOR[1] - pulse, FOOD_COLOR[2], 1.0];
                        compositor.draw_rounded_rect(RoundedRectParams {
                            x: px + 1.0, y: py + 1.0, w: sz - 2.0, h: sz - 2.0,
                            color: food_c, corner_radius: radius * 1.2, border_width: 0.0, border_color: [0.0; 4],
                        });
                    }
                    Cell::Wall => {
    ...
    fn draw_head(&self, compositor: &mut Compositor, px: f32, py: f32, sz: f32, radius: f32) {
        compositor.draw_rounded_rect(RoundedRectParams {
            x: px, y: py, w: sz, h: sz,
            color: SNAKE_HEAD, corner_radius: radius * 1.5, border_width: 0.0, border_color: [0.0; 4],
        });
        // Eyes
        ...
        let (ex1, ey1, ex2, ey2) = match self.dir {
            (1, 0) => (cx_f + sz * 0.15, cy_f - sz * 0.15, cx_f + sz * 0.15, cy_f + sz * 0.15),
            (-1, 0) => (cx_f - sz * 0.15, cy_f - sz * 0.15, cx_f - sz * 0.15, cy_f + sz * 0.15),
            (0, -1) => (cx_f - sz * 0.15, cy_f - sz * 0.15, cx_f + sz * 0.15, cy_f - sz * 0.15),
            _ => (cx_f - sz * 0.15, cy_f + sz * 0.15, cx_f + sz * 0.15, cy_f + sz * 0.15),
        };
        compositor.draw_rounded_rect(RoundedRectParams {
            x: ex1 - eye_r, y: ey1 - eye_r, w: eye_r * 2.0, h: eye_r * 2.0,
            color: [0.05, 0.05, 0.10, 1.0], corner_radius: eye_r, border_width: 0.0, border_color: [0.0; 4],
        });
        compositor.draw_rounded_rect(RoundedRectParams {
            x: ex2 - eye_r, y: ey2 - eye_r, w: eye_r * 2.0, h: eye_r * 2.0,
            color: [0.05, 0.05, 0.10, 1.0], corner_radius: eye_r, border_width: 0.0, border_color: [0.0; 4],
        });
    }
