//! anm player: plays a .anm file with `anm::AnmPlayer` on a plev
...
//!
//! Run: `cargo run --release --example anm_player -- file.anm`
//! Space pauses, left/right arrows scrub by half a second; the
//! animation loops.

use std::sync::Arc;

use anm::asset_path::unpack;
use anm::{AnmPlayer, AssetKind, LoweredAsset};
...
impl ApplicationHandler for AnmApp {
...
fn main() {
    ...
    let Some(path) = std::env::args().nth(1) else {
        eprintln!("usage: anm_player <file.anm>");
        std::process::exit(2);
    };
    ...
    let doc = match anm::decode(&bytes) {
        Ok(d) => d,
        Err(e) => {
    ...
    let (sw, sh) = anm::stage_size(&doc.descs).unwrap_or((WINDOW, WINDOW));
    ...
    let assets: Vec<LoweredAsset> = doc
        ...
        .collect();
    let mut player = match AnmPlayer::new(doc.timeline) {
        Ok(p) => p,
        Err(e) => {
    ...
    let title = format!(
        "{file} - {kb:.0} KB - anm v0 ({:.1}s, {} keyframes)",
        player.duration_s(),
