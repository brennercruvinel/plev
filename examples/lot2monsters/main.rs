//! lot2monsters: convert lottie json to our .monster format on the command
//! line. The json is read exactly once, here; whatever plays the
//! output never sees it.
//!
//! Run: `cargo run --release --example lot2monsters -- in.json [out.monster]`
//! (default output: in.monster next to the input). Prints the measured
//! byte table: json vs monster, asset/timeline breakdown.

fn main() {
    env_logger::init();
    let mut args = std::env::args().skip(1);
    let Some(input) = args.next() else {
        eprintln!("usage: lot2monsters <in.json> [out.monster]");
        std::process::exit(2);
    };
    ...
    println!(
        ...
        100.0 * stats.asset_bytes as f64 / stats.monster_bytes as f64
    );
    println!(
        "  bytes: json {:.1} KB -> monster {:.1} KB ({:.2}x)",
        json_kb,
        monster_kb,
        monster_kb / json_kb
    );
