...
impl TodoApp {
    ...
    pub(crate) fn render(&mut self) {
        ...
        self.compositor.draw_text(
            TextNodeKey::new("phi todos", 28.0, 36.0, Some(cw)),
            cx,
            16.0,
            ACCENT,
        );
        self.compositor.draw_text(
            TextNodeKey::new("Proof of Life Demo", 12.0, 16.0, Some(cw)),
            cx,
            50.0,
            TEXT_DIM,
        );
    ...
    fn build_item_list(&mut self, cx: f32, cw: f32, vw: f32, vh: f32) {
        ...
        for (vi, (id, text, completed, opacity)) in visible_data.iter().enumerate() {
            ...
            self.compositor.draw_text(
                TextNodeKey::new(text, 15.0, 20.0, Some(text_w)),
                text_x,
                iy + (ih - 15.0) / 2.0,
                text_color,
            );
        ...
        if visible_data.is_empty() {
            ...
            self.compositor.draw_text(
                TextNodeKey::new(msg, 14.0, 20.0, Some(cw)),
                cx + 12.0,
                ey,
                TEXT_DIM,
            );
    ...
    fn build_footer(&mut self, cx: f32, cw: f32, vw: f32, vh: f32) {
        ...
        self.compositor.draw_text(
            TextNodeKey::new(&counter_text, 13.0, 17.0, Some(200.0)),
            cx,
            fy + 4.0,
            TEXT_DIM,
        );
        ...
        for (f, fx, ffy, fw, fh) in self.filter_rects() {
            ...
            self.compositor.draw_text(
                TextNodeKey::new(f.label(), 12.0, 16.0, Some(fw - 10.0)),
                fx + 10.0,
                ffy + 6.0,
                color,
            );
