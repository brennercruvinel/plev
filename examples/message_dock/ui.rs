...
impl AnimatedDock {
    ...
    pub(crate) fn update_animations(&mut self) {
        ...
        for i in 0..NUM_CHARS {
            ...
            let target_o = if is_expanded && !is_selected {
                0.0
            } else if is_expanded && i == 0 {
                0.0
            } else {
                1.0
            };
    ...
    pub(crate) fn build_scene(
        ...
        if self.input_opacity > 0.03 {
            ...
            comp.draw_text(
                TextNodeKey::new(&placeholder, 14.0, 20.0, Some(max_w)),
                text_x,
                text_y,
                [
                    TEXT_PLACEHOLDER[0],
    ...
    fn build_send_button(
        ...
        if is_expanded && self.send_btn_opacity > 0.03 {
            ...
            let btn_color = if self.send_hovered {
                SEND_BTN_HOVER
            } else {
                SEND_BTN
            };
            ...
            for j in 0..3 {
                draw_rect(
                    comp,
                    ix,
                    px(start_y + j as f32 * (bar_h + bar_gap)),
                    bar_w,
                    bar_h,
                    [0.55, 0.55, 0.65, 0.8 * menu_opacity],
                );
            }
    ...
    fn build_chrome(&self, comp: &mut Compositor, vw: f32, vh: f32, frame: u64) {
        ...
        comp.draw_text(
            TextNodeKey::new(
                "GPU-first compositing  //  wgpu 28  //  6 platforms",
                14.0,
                20.0,
                Some(vw - 80.0),
            ),
        ...
        comp.draw_text(
            TextNodeKey::new(
                "Hover and click the characters in the dock below",
                13.0,
                18.0,
                Some(vw - 80.0),
            ),
