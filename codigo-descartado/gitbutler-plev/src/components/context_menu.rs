:compositor::{Compositor, LayerId, SceneNode, TextNodeKey};
...
///
/// `x`, `y` is the top-left of the menu (from [`::overlay::Overlay`]).
/// Returns `(menu_w, menu_h, item_rects)` so the caller can call
/// [`::overlay::OverlayManager::set_bounds`] on the first frame.
pub fn draw(
