//! GitButler UI — phi native port
//!
//! GPU-native 3-panel workspace UI (no Tauri, no WebView, no JavaScript).
//! Built on phi: wgpu + winit, single Rust binary.
//!
//! Run: `cargo run -p gitbutler-phi`
...
use phi::texture_pool::TexturePool;
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        let attrs = WindowAttributes::default()
            .with_title("GitButler — Plev native")
        ...
        self.state = GpuState::Ready { gpu, text_system, _pool: pool };
        ...
        match event {
            ...
            WindowEvent::CursorMoved { position, .. } => {
                ...
                match self.workspace.hit_test_handle(cx) {
                    Some(Side::Left)  => { self.workspace.hover_left_handle = true;  self.workspace.hover_right_handle = false; }
                    Some(Side::Right) => { self.workspace.hover_right_handle = true; self.workspace.hover_left_handle = false;  }
                    None              => { self.workspace.hover_left_handle = false;  self.workspace.hover_right_handle = false; }
                }

                let on_handle = self.workspace.hover_left_handle || self.workspace.hover_right_handle
                    || self.workspace.dragging_left || self.workspace.dragging_right;
                if let Some(w) = &self.window {
                    w.set_cursor(if on_handle { CursorIcon::EwResize } else { CursorIcon::Default });
                }

                let handle_changed = old_lh != self.workspace.hover_left_handle || old_rh != self.workspace.hover_right_handle;
