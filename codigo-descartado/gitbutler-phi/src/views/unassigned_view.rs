...
impl UnassignedView {
    ...
    pub fn hit_test(&self, cx: f32, cy: f32) -> Option<usize> {
        self.hit_rects.iter().position(|(rx, ry, rw, rh)| {
    ...
    pub fn select(&mut self, idx: Option<usize>) -> bool {
        if self.selected_idx == idx { return false; }
    ...
    pub fn render(
        ...
        for (i, file) in self.files.iter().enumerate() {
            ...
            compositor.push(SceneNode::Rect {
                x, y: item_y, w, h: ITEM_H,
                color: row_bg.to_array(),
            });

            // Status badge (single letter)
            compositor.push(SceneNode::Text {
                key: TextNodeKey::new(file.status.label(), FONT_SIZE, FONT_SIZE * 1.3, None).with_weight(700),
                x: x + PAD_X,
            ...
            compositor.push(SceneNode::Text {
                key: TextNodeKey::new(&display_name, FONT_SIZE, FONT_SIZE * 1.3, Some(w - STATUS_W - PAD_X * 3.0)).with_weight(400),
                x: x + PAD_X + STATUS_W,
                y: item_y + (ITEM_H - FONT_SIZE * 1.3) / 2.0,
                color: if is_selected { theme.text_1 } else { theme.text_2 }.to_array(),
            });
...
fn mock_files() -> Vec<FileEntry> {
    vec![
        FileEntry { path: "src/compositor.rs".into(), status: FileStatus::Modified },
        FileEntry { path: "src/builder.rs".into(), status: FileStatus::Modified },
        FileEntry { path: "src/scroll.rs".into(), status: FileStatus::Added },
        ...
        FileEntry { path: "shaders/quad.wgsl".into(), status: FileStatus::Modified },
        FileEntry { path: "shaders/text.wgsl".into(), status: FileStatus::Modified },
        FileEntry { path: "crates/gitbutler-plev/src/main.rs".into(), status: FileStatus::Added },
        FileEntry { path: "crates/gitbutler-plev/src/theme.rs".into(), status: FileStatus::Added },
        FileEntry { path: "assets/fonts/Inter-Regular.ttf".into(), status: FileStatus::Added },
        FileEntry { path: "mission/readme.md".into(), status: FileStatus::Modified },
        FileEntry { path: "mission/steps/ongoing/TASK-42.md".into(), status: FileStatus::Added },
        FileEntry { path: "src/components/button.rs".into(), status: FileStatus::Deleted },
        FileEntry { path: "src/lib.rs".into(), status: FileStatus::Modified },
        FileEntry { path: "Cargo.lock".into(), status: FileStatus::Modified },
    ]
}
