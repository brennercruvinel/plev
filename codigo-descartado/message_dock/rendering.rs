...
impl App {
    ...
    fn render(&mut self) {
        ...
        let GpuState::Ready { ref mut gpu, ref mut text_system } = self.state else { return };
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _: WindowId, event: WindowEvent) {
        match event {
            ...
            WindowEvent::Resized(size) => {
                if let GpuState::Ready { ref mut gpu, .. } = self.state { gpu.resize(size.width, size.height); }
                if let Some(ref w) = self.window { w.request_redraw(); }
            }
            WindowEvent::CursorMoved { position, .. } => {
                self.input_state.handle_cursor_moved(position.x as f32, position.y as f32);
                if let Some(ref w) = self.window { w.request_redraw(); }
            }
