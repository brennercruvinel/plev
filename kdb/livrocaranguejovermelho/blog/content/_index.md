...
Brenner is an opinionated, modern, pretty, and clean [Zola](https://www.getzola.org) theme that has the purpose of greatly simplifying the process of rolling up your blog. It aims to provide all the necessary options for comfortable writing, while keeping the balance of simplicity.
...
Some of the features Brenner has to offer:
...
{% alert(tip=true) %}
If you use Brenner and enjoy it, or just like my work, please consider [buying me a coffee](https://github.com/brennercruvinel), it would mean the world to me ^^.
{% end %}
...
```bash
git submodule init
git submodule add https://github.com/brennercruvinel.git themes/brennercruvinel.com
```

Otherwise, simply clone it to your `themes` directory:

```bash
git clone https://github.com/brennercruvinel.git themes/brennercruvinel.com
```
...
Brenner offers some configuration options to make it fit you better; most options have pretty descriptive comments, so it should be easy to understand what they do.

## Front Matter

Brenner has some [front matter](https://www.getzola.org/documentation/content/page/#front-matter) variables that you can use by setting them in the `[extra]` section:
...
Brenner ships with a localization system based on one used in [tabi](https://github.com/welpo/tabi), it's very easy to use and quite flexible at the same time.
...
Inside that file, copy-paste one of the existing translations from Brenner and adapt it to your needs. You can also check [tabi](https://github.com/welpo/tabi/tree/main/i18n) translation files for reference.

Additionally to translating Brenner, you can also override the English stings by copy-pasting `en.toml` from Brenner to the `i18n` directory of your website and adjusting the values to your liking.
...
If for some reason overridden style is not respected, try using `!important` (don't use it unless needed). You can import styles from Brenner using:

```scss
@use "../themes/brennercruvinel.com/sass/NEEDED_FILE.scss";
```
...
Brenner respects chosen accent color everywhere. To use your own, simply change it in `config.toml`:
...
- Yours? <small>(feel free to [contact me](https://github.com/brennercruvinel/find/#contacts) or send a pull request)</small>
...
All sources for Brenner's assets are available [here](https://github.com/brennercruvinel) and licensed under CC BY-SA 4.0. The reason for not putting the sources in the same repo as Brenner itself is simple: I want it to be as small as possible, so that repo cloning is fast and doesn't make the site significantly heavier; this is also why the demo uses remote images instead of local copies.
...
- [Dexter Reed](https://sungsphinx.codeberg.page) for being the first Brenner user, preventing it from being axed.
